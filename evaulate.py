...
def segmentVoxel(imgLoc, model):
    ...
    # for docker we use the name 'tumor_GBMNet18_class.nii.gz'
    name = 'tumor_GBMNet18_class.nii.gz'
...
def make_preds():
    ...
    folders = [x for x in folders if 'flair.nii' in x]
    writeText = open('./data/test.txt', 'w')
    for folder in folders:
        files = glob.glob('./data/*flair.nii.gz')
        if len(files) <= 0:
            continue
        writeText.write(files[0] + ', ' + '\n')
    writeText.close()
...
if __name__ == '__main__':
    ...
    data_dir = './data/' # evaluate on original data and not the processed one
    ...
    #exit()
    with open(test_file) as txtFile:
        for line in txtFile:
            line_arr = line.split(',')
            img_file = line_arr[0]
